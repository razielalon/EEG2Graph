...
`subject_idx` (subject-conditioned models only): held to the OUTPUT slot for all
three conditions, so only the EEG varies. The EEG ablation therefore isolates the
EEG's causal contribution given the same subject context — the subject embedding
is never the thing being ablated.
...
def subject_bucket(subject_id, n_buckets):
    """md5(subject_id) % n_buckets — must match the training-time bucketing."""
...
def generate_condition(model, eeg, meta, tokenizer, device, perm, mode,
    ...
    a derangement for `swap`). mode in {real, swap, zero}. has_fixation follows
    the EEG (perm[i]); subject_idx is held to the output slot (i).
    """
    n_buckets = (model.subject_emb.num_embeddings
                 if getattr(model, "subject_emb", None) is not None else 0)
    seqs, preds = [], []
    N = len(eeg)
    for start in range(0, N, batch_size):
        ...
        gen_kwargs = dict(max_len=max_len, num_beams=num_beams)
        if n_buckets > 0:
            subj = [subject_bucket(meta[i].get("subject_id", ""), n_buckets) for i in sl]
            gen_kwargs["subject_idx"] = torch.tensor(subj, dtype=torch.long, device=device)
        gen = model.generate(src, src_mask, **gen_kwargs)
...
def main():
    ...
    if getattr(model, "subject_emb", None) is not None:
        print(f"subject-conditioned model: {model.subject_emb.num_embeddings} buckets "
              f"(subject_idx held to output slot for all conditions)")
